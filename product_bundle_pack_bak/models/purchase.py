...
class PurchaseOrderLine(models.Model):
    ...
    def recalculate_product_pack(self):
        """
        Recalculate the product pack details and price after the purchase order is created.
        """
        for line in self:
            if line.product_id.is_pack:
                total_price = 0.0
                for pack_item in line.product_id.pack_ids:
                    qty = pack_item.qty_uom
                    price = pack_item.product_id.list_price
                    total_price += qty * price

                # Update the unit price of the purchase order line
                line.write({
    ...
    def onchange_product_id(self):
        """
        Override the onchange method to handle product packs.
        """
        res = super(PurchaseOrderLine, self).onchange_product_id()
        if self.product_id.is_pack:
            self.recalculate_product_pack()
        return res

    def write(self, vals):
        """
        Override the write method to recalculate product pack when changes are made.
        """
        res = super(PurchaseOrderLine, self).write(vals)
        if 'product_id' in vals or 'product_qty' in vals:
            for line in self:
                if line.product_id.is_pack:
                    line.recalculate_product_pack()
        return res
