...
class StockRequest(models.Model):
    ...
    def _check_confirm_permission(self):
        """Check if current user can confirm stock request"""
        if not self.env.user.has_group('stock_request.group_stock_request_manager'):
            raise UserError(_("Only Stock Request Managers can confirm stock requests."))

    def action_confirm(self):
        self._check_confirm_permission()
    ...
    def _action_launch_procurement_rule(self):
        """
        Launch procurement group run method dengan error handling untuk vendor validation
        """
        precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure"
        )
        ...
        for request in self:
            ...
            try:
                ...
                # Handle vendor-related errors untuk stock request
                if any(keyword in error_msg.lower() for keyword in [
                    'vendor', 'supplier', 'price', 'purchase', 'no matching', 
                    'supplierinfo', 'seller', 'buy'
                ]):
                    # Create purchase order manually
                    try:
                        request._create_manual_purchase_order()
                        # Log warning message
                        request.message_post(
                            body=_("Purchase order created manually due to missing vendor information. Please review and update vendor details."),
                            message_type='comment'
                        )
                        continue
                    except Exception as e:
                        # Jika manual creation juga gagal, log error dan lanjutkan
                        request.message_post(
                            body=_("Warning: Could not create purchase order automatically. Error: %s") % str(e),
                            message_type='comment'
                        )
                        continue
                else:
    ...
    def _create_manual_purchase_order(self):
        """Create purchase order manually untuk bypass vendor validation"""
        
        # Get or create generic vendor
        generic_vendor = self._get_or_create_generic_vendor()
        
        # Check existing PO untuk vendor yang sama
        existing_po = self.env['purchase.order'].search([
            ('partner_id', '=', generic_vendor.id),
            ('state', 'in', ['draft', 'sent']),
            ('company_id', '=', self.company_id.id),
        ], limit=1)
        ...
        pol_vals = {
            ...
            'price_unit': 0.0,  # Will be filled manually
            'date_planned': self.expected_date,
            'stock_request_ids': [(4, self.id)],
        }
